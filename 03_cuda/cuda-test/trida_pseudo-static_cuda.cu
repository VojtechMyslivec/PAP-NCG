...
CUDA_DEV void cVektor::setHodnoty( ) {
    int blok  = blockIdx.x;
...
void cudaVektorInit( cVektor *& device, const cVektor * host, int * devStatickePole ) {
    if ( host == NULL ) {
        cerr << "cudaVektorUklid(): host nesmi by NULL" << endl;
        throw "cudaVektorUklid(): host nesmi by NULL";
    }
    ...
    // zkopruje ukazatel na hodnoty pseudo-statickeho pole
    HANDLE_ERROR( 
       cudaMemcpy( 
                   &(device->statickePole),
                   &(devStatickePole),
...
__global__ void wrapperSectiVektory( cVektor * devA, const cVektor * devB ) {
    printf( "CUDA HELLO\n" );
    devA->setHodnoty( );
...
int main( void ) {
    ...
    cVektor * devA,  * devB;
    cudaVektorInit( devA, hostA, devPole );
    cudaVektorInit( devB, hostB, devPole );

    int bloku = (DELKA + BLOK_VELIKOST - 1)/BLOK_VELIKOST;

    wrapperSectiVektory<<<bloku,BLOK_VELIKOST>>>( devA, devB );
